...
from modeling_ring_llama import RingLlamaForCausalLM
...
if __name__ == "__main__":
    ...
    parallel_model = RingLlamaForCausalLM(cfg).to(device)
    print(
        ...
        with torch.no_grad():
            ...
            with ctx as prof:
                ...
                for step in range(num_iterations):
                    ...
                    # Compute the range for the current rank
                    chunk_size = seq_len // world_size
                    input_ids = torch.randint(vocab_size, (batch_size, chunk_size))
                    start = rank * chunk_size
                    end = start + chunk_size

                    # Generate the pos_id for the current rank
                    position_ids = (
