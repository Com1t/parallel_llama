...
if __name__ == "__main__":
    ...
    temp_mask = torch.ones(seq_len, seq_len, dtype=torch.bool, device=device).tril(
        diagonal=0
    )
    ...
    query_list = torch.split(master_query, master_query.shape[2] // world_size, dim=2)
    key_list = torch.split(master_key, master_key.shape[2] // world_size, dim=2)
    value_list = torch.split(master_value, master_value.shape[2] // world_size, dim=2)
    ...
    if rank == 0:
        ...
        if torch.allclose(sdpa_attn_output, final_attn_output, atol=1e-3, rtol=1e-3):
            print(
