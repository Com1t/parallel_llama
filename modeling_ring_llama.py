...
from attention import RingLlamaAttention
from mlp import LlamaMLP


class RingLlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        super().__init__()
        self.layer_idx = layer_idx
        self.hidden_size = config.hidden_size

        self.self_attn = RingLlamaAttention(config=config, layer_idx=layer_idx)
...
class RingLlamaModel(nn.Module):
    """
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [
                RingLlamaDecoderLayer(config, layer_idx)
...
class RingLlamaForCausalLM(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.model = RingLlamaModel(config)
        self.vocab_size = config.vocab_size
